...
export const AllGames: React.FC<Props> = ({ predictions, refreshPredictions }) => {
  ...
  const gamesToDisplay = useMemo(() => {
    return games.filter((game) => !predictions.some((prediction) => prediction.gameId === game.id));
  }, [predictions, games]);
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        {!loading && gamesToDisplay.map((game) => <Game key={game.id} game={game} onSuccess={refreshPredictions} />)}
      </div>
    </div>
  );
};
